...
const panelClass = 'border-0 bg-[#e8eef8]/90 shadow-[-12px_-12px_28px_rgba(255,255,255,0.95),12px_12px_28px_rgba(148,163,184,0.35)] backdrop-blur-sm';
const insetCardClass = 'rounded-2xl border-0 bg-[#e3ebf7] shadow-[inset_-8px_-8px_18px_rgba(255,255,255,0.85),inset_8px_8px_18px_rgba(148,163,184,0.3)]';
const softInputClass = 'border-0 bg-[#edf3fb] text-slate-700 shadow-[inset_-4px_-4px_10px_rgba(255,255,255,0.9),inset_4px_4px_10px_rgba(148,163,184,0.28)] focus-visible:ring-2 focus-visible:ring-indigo-300';
const softSelectClass = 'h-10 w-full rounded-md border-0 bg-[#edf3fb] px-3 text-sm text-slate-700 shadow-[inset_-4px_-4px_10px_rgba(255,255,255,0.9),inset_4px_4px_10px_rgba(148,163,184,0.28)] focus:outline-none focus:ring-2 focus:ring-indigo-300';
...
const ElectionsPage = () => {
  ...
  if (!accessLoading && electionsClosed) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#edf3fb] via-[#e7eef9] to-[#dfe8f6]">
        <Navbar />
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#edf3fb] via-[#e7eef9] to-[#dfe8f6] text-slate-700">
      <Navbar />
      <div className="container mx-auto px-4 py-8 space-y-6">
        <div className="relative overflow-hidden rounded-3xl border-0 bg-[#e8eef8]/90 p-8 shadow-[-16px_-16px_32px_rgba(255,255,255,0.95),16px_16px_32px_rgba(148,163,184,0.35)] backdrop-blur-xl">
          <div className="absolute -right-16 -top-16 h-56 w-56 rounded-full bg-indigo-200/60 blur-3xl" />
          <div className="absolute -bottom-16 -left-16 h-56 w-56 rounded-full bg-sky-200/70 blur-3xl" />
          <p className="text-sm uppercase tracking-[0.3em] text-slate-500">Private Club Election Portal</p>
          <h1 className="mt-2 text-3xl font-bold md:text-4xl">Executive Elections Command Center</h1>
          <p className="mt-2 max-w-2xl text-slate-600">Luxury-grade election workflow with strict phase controls, audited voting, and transparent publication of candidates and results.</p>
          <div className="mt-5 flex flex-wrap gap-3 text-xs">
            <Badge className="bg-[#edf3fb] text-slate-600 shadow-[inset_-3px_-3px_8px_rgba(255,255,255,0.9),inset_3px_3px_8px_rgba(148,163,184,0.25)] hover:bg-[#f3f7fd]"><Sparkles className="mr-1 h-3.5 w-3.5" /> Premium Governance UX</Badge>
            <Badge className="bg-[#edf3fb] text-slate-600 shadow-[inset_-3px_-3px_8px_rgba(255,255,255,0.9),inset_3px_3px_8px_rgba(148,163,184,0.25)] hover:bg-[#f3f7fd]"><ShieldCheck className="mr-1 h-3.5 w-3.5" /> Controlled by Admin Phases</Badge>
            <Badge className="bg-[#edf3fb] text-slate-600 shadow-[inset_-3px_-3px_8px_rgba(255,255,255,0.9),inset_3px_3px_8px_rgba(148,163,184,0.25)] hover:bg-[#f3f7fd]"><BarChart3 className="mr-1 h-3.5 w-3.5" /> Live Turnout Analytics</Badge>
          </div>
        </div>
        ...
        {canManageElections(user) && (
          <Card className={panelClass}>
            ...
            <CardContent className="grid gap-3 md:grid-cols-2">
              <div className="space-y-1 md:col-span-2"><Label>Election Title</Label><Input className={softInputClass} value={title} onChange={(e) => setTitle(e.target.value)} /></div>
              <div className="space-y-1 md:col-span-2"><Label>Positions Open</Label><Input className={softInputClass} value={roles} onChange={(e) => setRoles(e.target.value)} /></div>
              <div className="space-y-1 md:col-span-2"><Label>Rules & Eligibility</Label><Textarea className={softInputClass} value={description} onChange={(e) => setDescription(e.target.value)} /></div>
              <div className="space-y-1"><Label>Notice Date</Label><Input className={softInputClass} type="date" value={notificationDate} onChange={(e) => setNotificationDate(e.target.value)} /></div>
              <div className="space-y-1"><Label>Nomination Start</Label><Input className={softInputClass} type="date" value={nominationStart} onChange={(e) => setNominationStart(e.target.value)} /></div>
              <div className="space-y-1"><Label>Nomination End</Label><Input className={softInputClass} type="date" value={nominationEnd} onChange={(e) => setNominationEnd(e.target.value)} /></div>
              <div className="space-y-1"><Label>Scrutiny Date</Label><Input className={softInputClass} type="date" value={scrutinyDate} onChange={(e) => setScrutinyDate(e.target.value)} /></div>
              <div className="space-y-1"><Label>Polling Date</Label><Input className={softInputClass} type="date" value={pollingDay} onChange={(e) => setPollingDay(e.target.value)} /></div>
              <div className="space-y-1"><Label>Result Date</Label><Input className={softInputClass} type="date" value={resultsDay} onChange={(e) => setResultsDay(e.target.value)} /></div>
              <div className="space-y-1"><Label>Withdrawal Deadline</Label><Input className={softInputClass} type="date" value={withdrawalDeadline} onChange={(e) => setWithdrawalDeadline(e.target.value)} /></div>
        ...
        <Card className={panelClass}>
          ...
          <CardContent className="space-y-2">
            ...
            {elections.map((item) => (
              <button key={item.election_id} onClick={() => setSelectedElectionId(item.election_id)} className={`w-full rounded-xl border p-3 text-left transition ${activeElection?.election_id === item.election_id ? 'border-0 bg-[#eef4fc] text-slate-700 shadow-[-6px_-6px_14px_rgba(255,255,255,0.85),6px_6px_14px_rgba(148,163,184,0.25)]' : 'border-0 bg-[#e3ebf7] text-slate-600 shadow-[inset_-5px_-5px_12px_rgba(255,255,255,0.8),inset_5px_5px_12px_rgba(148,163,184,0.24)] hover:bg-[#edf3fb]'}`}>
          ...
          <Card className={panelClass}>
            ...
            <CardContent className="grid gap-2 md:grid-cols-2">
              {[
                ...
                <label key={key} className="flex items-center justify-between rounded-xl border-0 bg-[#e3ebf7] p-3 text-sm text-slate-700 shadow-[inset_-5px_-5px_12px_rgba(255,255,255,0.8),inset_5px_5px_12px_rgba(148,163,184,0.24)]">
                  <span>{label}</span>
          ...
          <Card className={panelClass}>
            ...
            <CardContent className="grid gap-3 md:grid-cols-2">
              ...
              {nominationModuleActive && (
                <>
                  <div className="space-y-1"><Label>Player Name</Label><Input className={softInputClass} value={getActorName(user)} disabled /></div>
                  <div className="space-y-1"><Label>Player ID</Label><Input className={softInputClass} value={playerId} onChange={(e) => setPlayerId(e.target.value)} /></div>
                  <div className="space-y-1"><Label>Position Applied</Label>
                    <select className={softSelectClass} value={nominationRole} onChange={(e) => setNominationRole(e.target.value)}>
                      <option value="">Select Position</option>
                      {electionRoles.map((role) => <option key={role} value={role}>{role}</option>)}
                    </select>
                  </div>
                  <div className="space-y-1"><Label>Proposer</Label><Input className={softInputClass} value={proposer} onChange={(e) => setProposer(e.target.value)} /></div>
                  <div className="space-y-1"><Label>Seconder</Label><Input className={softInputClass} value={seconder} onChange={(e) => setSeconder(e.target.value)} /></div>
                  <div className="space-y-1 md:col-span-2"><Label>Manifesto</Label><Textarea className={softInputClass} value={manifesto} onChange={(e) => setManifesto(e.target.value)} /></div>
          ...
          <Card className={panelClass}>
            ...
            <CardContent className="space-y-2">
              ...
              {myNominations.map((item) => (
                <div key={nominationKey(item)} className={insetCardClass + " p-3"}>
          ...
          <Card className={panelClass}>
            ...
            <CardContent className="space-y-2">
              {nominations.filter((item) => item.election_id === activeElection.election_id).map((item) => (
                <div key={nominationKey(item)} className={insetCardClass + " p-3 space-y-2"}>
                  ...
                  <Input className={softInputClass} placeholder="Remarks / reason" value={reviewRemarks[nominationKey(item)] || ''} onChange={(e) => setReviewRemarks((prev) => ({ ...prev, [nominationKey(item)]: e.target.value }))} />
          ...
          <Card className={panelClass}>
            ...
            <CardContent className="space-y-3">
              {electionRoles.map((role) => (
                <div key={role} className={insetCardClass + " p-3"}>
                ...
                <>
                  {electionRoles.map((role) => (
                    <div key={role} className="space-y-1">
                      <Label>{role}</Label>
                      <select className={softSelectClass} value={voteSelections[role] || ''} onChange={(e) => setVoteSelections((prev) => ({ ...prev, [role]: e.target.value }))}>
        ...
        {activeElection && canManageElections(user) && (
          <Card className={panelClass}>
            ...
            <CardContent className="grid gap-2 md:grid-cols-2">
              <div className={insetCardClass + " p-3"}>Eligible Players: {eligibleVoters.size}</div>
              <div className={insetCardClass + " p-3"}>Votes Cast: {uniqueVoters.size}</div>
              <div className={insetCardClass + " p-3"}>Pending Voters: {Math.max(eligibleVoters.size - uniqueVoters.size, 0)}</div>
              <div className={insetCardClass + " p-3"}>Turnout: {turnoutPercent}%</div>
              <div className={insetCardClass + " p-3 md:col-span-2"}>Live vote submission count: {activeVotes.length}</div>
          ...
          <Card className={panelClass}>
            ...
            <CardContent className="space-y-2">
              <div className="grid gap-2 md:grid-cols-2">
                <div className="space-y-1"><Label>Term Start</Label><Input className={softInputClass} type="date" value={termStart} onChange={(e) => setTermStart(e.target.value)} /></div>
                <div className="space-y-1"><Label>Term End</Label><Input className={softInputClass} type="date" value={termEnd} onChange={(e) => setTermEnd(e.target.value)} /></div>
              </div>
        ...
        <Card className={panelClass}>
          ...
          <CardContent className="grid gap-2 md:grid-cols-2">
            {terms.slice(0, 8).map((term) => (
              <div key={term.assignment_id} className={insetCardClass + " p-3"}>
