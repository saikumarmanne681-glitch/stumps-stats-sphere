...
export function AdminMessages() {
  const { messages, players, addMessage } = useData();
            ...
            <div key={msg.id} className="border rounded-lg p-3">
              <div className="flex justify-between items-center mb-1">
                <span className="text-sm font-medium">
                  {msg.from_id === 'admin' ? '→' : '←'} {msg.from_id === 'admin' ? `To: ${msg.to_id === 'all' ? 'All Players' : players.find(p => p.player_id === msg.to_id)?.name || msg.to_id}` : `From: ${players.find(p => p.player_id === msg.from_id)?.name || msg.from_id}`}
                </span>
