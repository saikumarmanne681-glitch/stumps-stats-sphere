...
import { Loader2, FileJson, Shield, ShieldCheck, ShieldX, Lock, Eye, Download, CheckCircle2 } from "lucide-react";
...
export function AdminScorelists() {
  ...
  const handleCertify = async (sl: DigitalScorelist, stage: string) => {
    ...
    await v2api.updateScorelist({ ...sl, certification_status: newStatus, certifications_json: JSON.stringify(certs), locked });
      ...
      <Dialog open={!!viewScorelist} onOpenChange={o => { if (!o) { setViewScorelist(null); setVerifyResult(null); } }}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto scrollbar-thin">
          {viewScorelist && (() => {
            ...
            // Calculate team scores from batting data
                ...
                <div className="relative z-10 space-y-6">
                  ...
                  {match && (
                    <>
                      ...
                      {/* Enhanced Scoreboard */}
                      <div className="grid grid-cols-3 gap-4 text-center bg-gradient-to-r from-primary/5 via-transparent to-primary/5 rounded-xl p-6 border">
                        <div>
                          ...
                          {!match.team_a_score && <p className="text-xs text-muted-foreground">({calcScore(match.team_a).overs} ov)</p>}
                        </div>
                        ...
                        <div>
                          ...
                          {!match.team_b_score && <p className="text-xs text-muted-foreground">({calcScore(match.team_b).overs} ov)</p>}
                        </div>
                      </div>
                    </>
                  )}
                  ...
                  {/* Certification Panel with Timeline */}
                          ...
                          return (
                            <div key={stage} className={`flex items-center gap-3 p-2 rounded text-sm ${cert ? 'bg-primary/5 border border-primary/20' : 'opacity-40'}`}>
                              ...
                              <div className="flex-1">
                                ...
                                {cert && <p className="text-xs text-muted-foreground">{cert.approver_name} – {cert.designation} • {new Date(cert.timestamp).toLocaleString()}</p>}
                              </div>
                              {cert && <Badge variant="outline" className="text-xs font-mono">{cert.token.substring(0, 12)}</Badge>}
                            </div>
                          );
                        })}
                      </div>
                  ...
                  <div className="border-t pt-4 text-center space-y-1">
                    ...
                    <p className="text-[10px] text-muted-foreground/60 italic mt-2">
                      Official League Record • Tampering Invalidates Document • This document is digitally certified.
                    </p>
                  </div>
                </div>
              </div>
